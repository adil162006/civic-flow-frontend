    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="grid flex-1 place-items-center bg-[#f5fbf5] px-5">
        ...
        >
          ...
          <div className="mt-4 rounded-md border border-emerald-200 bg-emerald-50/80 p-3 text-xs text-emerald-900 font-mono">
            <p className="font-bold">Demo Admin Credentials:</p>
            <p>Email: admin@civicflow.ai</p>
...
function AdminControlCenter({ activeDepartment }) {
  ...
  const departmentQueryParam = activeDepartment ? activeDepartment.apiValue : undefined
  const viewingLabel = activeDepartment ? activeDepartment.label : 'All Departments'

  const queryParams = new URLSearchParams()
  if (departmentQueryParam) queryParams.set('department', departmentQueryParam)
  ...
  const departmentCountQueries = useQueries({
    queries: DEPARTMENTS.map((dept) => ({
      queryKey: ['adminDeptCount', dept.slug],
                    ...
                    {complaints.isLoading ? (
                      ...
                      complaints.data?.complaints?.map((c) => (
                        <tr className="border-t border-slate-100" key={c._id}>
                          ...
                          <td>{c.category}</td>
                          <td>{typeof c.location === 'object' ? c.location?.address : c.location}</td>
